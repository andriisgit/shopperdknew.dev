...
class NWT_KCO_Model_Checkout extends Mage_Checkout_Model_Type_Onepage
{
   ...
   /**
    ...
    public function isTestMode()              { return $this->_helper->isTestMode(); }


    protected function throwRedirectToCartException($message) { throw new NWT_KCO_Exception($message,'checkout/cart'); }
    protected function throwRedirectToHomeException($message) { throw new NWT_KCO_Exception($message,'/'); }
   ...
   public function initCheckout($reloadIfCurrencyChanged = true) {
        ...
        if(!$shippingAddress->getPostcode()) {
            ...
            if($this->isTestMode() && !empty($locale['test']['postal_code'])) {
    ...
    public function checkCart() {
        ...
        if (!$quote->hasItems() || $quote->getHasError()) {
            if (Mage::helper('nwtkco')->checkIfRedirectToHomepage()) {
                $this->throwRedirectToHomeException();
            }
            else {
   ...
   public function updateKlarnaOrder()
   {
    ...
    protected function _prepareNewCustomerQuote()
    {
        ...
        if($billing->getTaxvat()){
            $customer->setTaxvat($billing->getTaxvat());
        }
    ...
    public function placeOrder($klarnaOrder,$kID) {
        ...
        if($klarnaData['customer']['type'] == NWT_KCO_Model_Klarna_Order::CUSTOMER_TYPE_COMPANY){
            $orgNr = $klarnaData['customer'][NWT_KCO_Model_Klarna_Order::COMPANY_ORG_NR_KEY];
            
            $klarnaData['shipping_address'][NWT_KCO_Model_Klarna_Order::COMPANY_ORG_NR_KEY] = $orgNr;
            $klarnaData['billing_address'][NWT_KCO_Model_Klarna_Order::COMPANY_ORG_NR_KEY] = $orgNr;
      ...
      //  print_r($klarnaData);
       // die;
        ...
        $billingAddress = $quote->getBillingAddress()
            ->addData($klarnaOrder->mageAddress($billing,$is_company))
            ->setCustomerAddressId(0)
            ->setSaveInAddressBook(0)
            ->setShouldIgnoreValidation(true);

        $shippingAddress = $quote->getShippingAddress()
            ->addData($klarnaOrder->mageAddress($shipping,$is_company))
            ->setSameAsBilling(1)
            ->setCustomerAddressId(0)
            ->setSaveInAddressBook(0)
            ->setShouldIgnoreValidation(true);
        ...
        // set company data to quote
        if($is_company){
            $quote->getBillingAddress()->setVatId($orgNr);
            $quote->getBillingAddress()->setCompany($billingAddress->getCompany());
            
            $quote->getShippingAddress()->setVatId($orgNr);
            $quote->getShippingAddress()->setCompany($shippingAddress->getCompany());
        ...
        if ($customer && $customer->getId()) {
            ...
            // save company info for a customer that already exist and doesn't have that info already!
            if ($is_company && $defaultBillingId = $customer->getDefaultBilling()){
                ...
                try{
                    ...
                    if(!$customerAddress->getCompany()){
                        $customerAddress->setCompany($billingAddress->getCompany()); $save = true;
                    }

                    if(!$customerAddress->getVatId()){
                        $customerAddress->setVatId($billingAddress->getTaxvat()); $save = true;
                    }
                    ...
                    if(!$customer->getTaxvat()){
                        $customer->setTaxvat($billingAddress->getTaxvat());
                        $customer->save();
                    }
                ...
                $quote->setCheckoutMethod(self::METHOD_CUSTOMER)
                    ->setCustomerTaxvat($billingAddress->getTaxvat());
            }
