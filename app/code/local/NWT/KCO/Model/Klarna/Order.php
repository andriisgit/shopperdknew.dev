...
class NWT_KCO_Model_Klarna_Order extends Klarna_Checkout_Order
{
    ...
    const COMPANY_ORG_NR_KEY = 'organization_registration_id';
    const COMPANY_NAME_KEY = 'organization_name';
    ...
    public function mageAddress($klrnAddr,$is_company=false) {

        // add fields for company!
        if($is_company){
            $this->_addrFieldMap['taxvat'] = self::COMPANY_ORG_NR_KEY;
            $this->_addrFieldMap['company'] = self::COMPANY_NAME_KEY;
        }
...
//         Mage::log('Quote total/taxes: '.$quote->getGrandTotal().' | '.$shippingAddress->getTaxAmount());
    ...
    public function updateFromQuote() {
        ...
        $customer = $quote->getCustomer();
        $orderData['customer']['type'] = $this->getDefaultCustomerType($customer);
        ...
        if($customer && $customer->getId()) {
            ...
            if($orderData['customer']['type'] == self::CUSTOMER_TYPE_COMPANY && $customer->getTaxvat()){
                $billingAddress =  $customer->getPrimaryBillingAddress();
                $orderData['customer'][self::COMPANY_ORG_NR_KEY] = $customer->getTaxvat();
                $orderData['shipping_address'][self::COMPANY_NAME_KEY] = $billingAddress->getCompany();
               
                $orderData['shipping_address']['given_name'] = $customer->getFirstname();
                $orderData['shipping_address']['family_name'] = $customer->getLastname();

                $orderData['shipping_address']['phone'] = $billingAddress->getTelephone();

                $orderData['shipping_address']['street_address'] = $billingAddress->getData('street');
                
                $orderData['shipping_address']['city'] = $billingAddress->getCity();
                
                $orderData['shipping_address']['country'] = strtolower($billingAddress->getCountryId());
    ...
    protected function _getDefaultOptions(){
        ...
        if($helper->getUseB2b()){
            $allowed_customer_types[] = self::CUSTOMER_TYPE_COMPANY;

            if($helper->getKeepB2c()){
                $allowed_customer_types[] = self::CUSTOMER_TYPE_PERSON;
            }
        ...
        $opt['allowed_customer_types'] = $allowed_customer_types;
       
       return $opt;
    }


    private function getDefaultCustomerType($customer=false){
        $helper = Mage::helper('nwtkco');
        ...
        // if b2b and b2c is active and customer clicks on one of the buttons, we will add that customer type...
        if(($helper->getAddButtonsInCheckout() && $customer_session_type = Mage::getSingleton('core/session')->getChangeKCOCustomerType())){
            if(in_array($customer_session_type,array(self::CUSTOMER_TYPE_PERSON,self::CUSTOMER_TYPE_COMPANY))){
                return $customer_session_type;
            }
        }


        // if customer is online and has company info, then return company type
        if($customer && $customer->getId() && $customer->getTaxvat()){
            return self::CUSTOMER_TYPE_COMPANY;
        }
