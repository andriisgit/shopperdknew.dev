...
class NWT_KCO_CheckoutController extends Mage_Checkout_Controller_Action
{
    ...
    public function companyAction(){
        if($this->possibleToSwitchType()){
            Mage::getSingleton('core/session')->setChangeKCOCustomerType(NWT_KCO_Model_Klarna_Order::CUSTOMER_TYPE_COMPANY);
        }
    ...
    public function privateAction(){
        if($this->possibleToSwitchType()){
            Mage::getSingleton('core/session')->setChangeKCOCustomerType(NWT_KCO_Model_Klarna_Order::CUSTOMER_TYPE_PERSON);
        }
    ...
    private function possibleToSwitchType(){
        $h = Mage::helper('nwtkco');
        return $h->isB2bAvailable() && $h->getKeepB2c() && $h->getAddButtonsInCheckout();
    }

    private function redirectToKco(){
        $this->_redirect(Mage::helper('nwtkco')->getCheckoutPath(),array('_secure' => true));
    }
