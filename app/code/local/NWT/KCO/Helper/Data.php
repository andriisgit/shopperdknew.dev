...
class NWT_KCO_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function b2bAllowedForLocale($store=null){
        $country = $this->getCountries($store);
        if(empty($country)) return false;

        $info = Mage::getSingleton('nwtkco/klarna_locale')->getCountry($country[0]);
        return isset($info["company_allowed"]) && $info["company_allowed"];
    }
    ...
    public function changeLayout($store = null) {
     ...
     public function checkIfRedirectToHomepage($store = null) {
        return Mage::getStoreConfigFlag(self::XML_NWTKCO_PATH.'tohomepage_after_items_removed',$store);
    }
