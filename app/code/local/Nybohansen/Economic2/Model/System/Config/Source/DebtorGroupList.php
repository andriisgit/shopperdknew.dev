...
class Nybohansen_Economic2_Model_System_Config_Source_DebtorGroupList {
 	...
 	public function toOptionArray(){
		...
		if($connection_result){
			$debtor_groups = $eapi->debtor_groups_get_all();
			$res = array();
			foreach ($debtor_groups as $debtor_group_number => $debtor_group_name) {
				$res[] = array('value' => $debtor_group_number, 'label' => $debtor_group_name);
			}
			return $res;	
		}else{
