...
class Queldorei_Shoppercategories_Adminhtml_ShoppercategoriesController extends Mage_Adminhtml_Controller_Action
{
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			$storeviews = array();
			if(!$data['stores'][0] || $data['stores'][0] == 0)
			{
				$storeViewCollection = Mage::app()->getStores();						
				foreach($storeViewCollection as $storeView)
				{				
					$storeviews[] = $storeView->getStoreId();
				}
				if(!empty($storeviews))
				{
					$data['stores'] = $storeviews;
				}				
			}
			...
			//Categories CSS
			Mage::getSingleton('shoppercategories/css')->regenerateCatCss($data);
	...
	public function deleteAction() {
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {				
				...
				$neededScheme = $model->getCollection()->addFieldToSelect('category_id')->addFieldToFilter('scheme_id', $this->getRequest()->getParam('id'))->getData();				
				Mage::getSingleton('shoppercategories/css')->removeAllShemeFiles($this->getRequest()->getParam('id'), $neededScheme[0]['category_id']);
        ...
        if(!is_array($shoppercategoriesIds)) {
            ...
            try {
                foreach ($shoppercategoriesIds as $shoppercategoriesId) {
					$neededScheme = Mage::getModel('shoppercategories/shoppercategories')->getCollection()->addFieldToSelect('category_id')->addFieldToFilter('scheme_id', $shoppercategoriesId)->getData();
					Mage::getSingleton('shoppercategories/css')->removeAllShemeFiles($shoppercategoriesId, $neededScheme[0]['category_id']);
