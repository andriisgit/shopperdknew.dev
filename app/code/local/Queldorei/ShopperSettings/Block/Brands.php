...
class Queldorei_ShopperSettings_Block_Brands extends Mage_Core_Block_Template
{
	public function getBrands()
	{
		...
		$brands = array();		
		if($brandsList && !is_array($brandsList))
		{
			$brands[] = array(
				'name' => $brandsList,
				'image' => $brandDir . str_replace(' ', '_', strtolower($brandsList)) . '.' . $brandExt,
				'url' => Mage::getUrl() . 'catalogsearch/result/?q=' . urlencode($brandsList),
			);
			return $brands;
		}
		else 
	...
	private function getBrandsWithProducts()
	{
		...
		if(Mage::getResourceModel('catalog/eav_attribute')->loadByCode('catalog_product',$attribute)->getId())
		{			
