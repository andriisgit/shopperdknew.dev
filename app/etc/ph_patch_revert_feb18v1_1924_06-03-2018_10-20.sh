#!/bin/bash
# This file reverts the bakup applied by Powerhosting 06-03-2018_10-20
initdir=$(pwd)
...
jscsslist_bu=$(tar -tf app/etc/ph_patch_backup_feb18v1_1924_06-03-2018_10-20.tar.gz | grep "\.js\|\.css" | cut -d/ -f2-)
...
cp app/etc/ph_patch_backup_feb18v1_1924_06-03-2018_10-20.tar.gz .
tar -xvf ph_patch_backup_feb18v1_1924_06-03-2018_10-20.tar.gz
rm -r var/cache/*
...
rm ph_patch_backup_feb18v1_1924_06-03-2018_10-20.tar.gz
rm /var/www/hifi-freaks.se/public_html/app/etc/ph_patch_backup_feb18v1_1924_06-03-2018_10-20.tar.gz
...
if [ "$revertmail" = "owner@example.com" ]; then
  ...
  curl -s http://resources.intl.powr.host/patches/ph/feb18v1/revertemail.html | sed -e "s~GENERALNAME~HiFi-freaks.se~g" -e "s|BASEURL|hifi-freaks.se|g" -e "s|PATCHDATE|06/03-18|g" -e "s|MAGEDIR|/var/www/hifi-freaks.se/public_html|g" -e "s/HOSTNAME/phhw-171004.intl.powerhosting.dk/g" -e "s/REVERTTIMESTAMP/${REVERTTIME}/g" | bsd-mailx -r 'patchbot@powerhosting.dk' -a 'Content-Type: text/html; charset=UTF-8' -a 'Content-Transfer-Encoding: binary' -a 'MIME-Version: 1.0' -s "Sikkerhedspatchen er netop rullet tilbage"     $revertmail
else "no valid email-address"
fi

cat << EOM >> app/etc/ph_patch.report
${REVERTTIME} /var/www/hifi-freaks.se/public_html feb18v1_1924 REVERTED
EOM
cd $initdir
rm -- "$0"
