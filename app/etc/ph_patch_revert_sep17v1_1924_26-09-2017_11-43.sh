#!/bin/bash
# This file reverts the bakup applied by Powerhosting 26-09-2017_11-43
initdir=$(pwd)
...
jscsslist_bu=$(tar -tf app/etc/ph_patch_backup_sep17v1_1924_26-09-2017_11-43.tar.gz | grep "\.js\|\.css" | cut -d/ -f2-)
...
cp app/etc/ph_patch_backup_sep17v1_1924_26-09-2017_11-43.tar.gz .
tar -xvf ph_patch_backup_sep17v1_1924_26-09-2017_11-43.tar.gz
rm -r var/cache/*
...
rm ph_patch_backup_sep17v1_1924_26-09-2017_11-43.tar.gz
rm /var/www/hifi-freaks.se/public_html/app/etc/ph_patch_backup_sep17v1_1924_26-09-2017_11-43.tar.gz

#Clear LB static cache for js and css elements
for file in $jscsslist_bu; do
  curl --connect-timeout 1 -s -H 'pragma: no-cache' -I -L http://hifi-freaks.se/$file > /dev/null
done
...
if [ "$revertmail" = "owner@example.com" ]; then
  ...
  curl -s http://resources.intl.powr.host/patches/ph/sep17v1/revertemail.txt | sed -e "s~GENERALNAME~HiFi-freaks.se~g" -e "s|BASEURL|hifi-freaks.se|g" -e "s|PATCHDATE|26/09-17|g" -e "s|MAGEDIR|/var/www/hifi-freaks.se/public_html|g" -e "s/HOSTNAME/phhw-130101.intl.powerhosting.dk/g" -e "s/REVERTTIMESTAMP/${REVERTTIME}/g" | mail     -r"patchbot@powerhosting.dk (Powerhosting PatchBot)"     -S ttycharset=utf-8 -S sendcharsets=utf-8 -S encoding=8bit     -s "Sikkerhedspatchen på hifi-freaks.se er netop rullet tilbage"     $revertmail
else "no valid email-address"
fi

cat << EOM >> app/etc/ph_patch.report
${REVERTTIME} /var/www/hifi-freaks.se/public_html sep17v1_1924 REVERTED
EOM
cd $initdir
rm -- "$0"
